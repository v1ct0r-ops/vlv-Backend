...
def registrar_rendicion_chofer(
    nombre_chofer: str,
    datos: RendicionCreate,
    ...
    try:
        ...
        rendicion = servicio.registrar_rendicion(nombre_chofer, datos, current.empresa_id)
...
def listar_rendiciones(
    ...
    query = (
        db.query(Rendicion)
        .filter(Rendicion.empresa_id == current.empresa_id)
...
def historial_rendiciones_chofer(
    ...
    query = db.query(Rendicion).filter(
        ...
        Rendicion.empresa_id == current.empresa_id,
    ).order_by(Rendicion.fecha.desc())
    return paginar(query, page)
...
def rendiciones_cerradas_chofer(
    ...
    servicio = RendicionService(db)
    return servicio.rendiciones_cerradas_chofer(nombre_chofer, current.empresa_id, mes, anio)


@router.get("/{rendicion_id}", response_model=RendicionRead)
def obtener_rendicion(
    rendicion_id: int,
    current: Usuario = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    try:
        servicio = RendicionService(db)
        rendicion = servicio.obtener_rendicion(rendicion_id, current.empresa_id)
        return servicio.formatear_rendicion(rendicion)
    except ValueError as e:
